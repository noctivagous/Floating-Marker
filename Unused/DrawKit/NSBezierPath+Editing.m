...
- (BOOL)subpathContainingElementIsClosed:(NSInteger)element
{
	...
	for (i = element; i < ec; ++i) {
		...
		if (et == NSBezierPathElementClosePath)
			return YES;
	}
...
- (NSInteger)partcodeHitByPoint:(NSPoint)p tolerance:(CGFloat)t startingFromElement:(NSInteger)startElement prioritiseOnPathPoints:(BOOL)onpPriority
{
	...
	for (i = startElement + 1; i < ec; ++i) {
		...
		if (et == NSBezierPathElementCurveTo) {
			if (onpPriority) {
				if (pet == NSBezierPathElementCurveTo) {
...
- (void)moveControlPointPartcode:(NSInteger)pc toPoint:(NSPoint)p colinear:(BOOL)colin coradial:(BOOL)corad constrainAngle:(BOOL)acon
{
	...
	CGFloat dx, dy;
	NSBezierPathElement previous = NSBezierPathElementMoveTo;
	NSBezierPathElement following = NSBezierPathElementMoveTo;
	NSPoint opp, centre;
	NSInteger prev, next;
	...
	if (et == NSBezierPathElementCurveTo) {
		...
		case 0: // control point 1
			if (colin) {
				if (previous == NSBezierPathElementCurveTo) {
					...
					[self setControlPoint:opp
							  forPartcode:prevPc];
				} else if (closedLoop && (previous == NSBezierPathElementMoveTo)) {
		...
		case 1: // control point 2
			if (colin) {
				if ((element < (ec - 1)) && (following == NSBezierPathElementCurveTo)) {
					...
					if (following == NSBezierPathElementCurveTo) {
						centre = [self controlPointForPartcode:partcodeForElementControlPoint(element, 2)];
		...
		[self setControlPoint:p
				  forPartcode:pc];
	} else if (et != NSBezierPathElementClosePath) {
		...
		if (depth == 1 && closedLoop && (et == NSBezierPathElementMoveTo) && colin) {
...
- (NSBezierPath*)deleteControlPointForPartcode:(NSInteger)pc
{
	...
	if (elem < 0 || (type == NSBezierPathElementCurveTo && aidx != 2))
		return self;
	else {
		...
		for (i = 0; i < m; ++i) {
			...
			if (i == elem) {
				...
				if (deletedFirstPoint && i == 1) {
					if (lm == NSBezierPathElementCurveTo)
...
- (NSBezierPath*)insertControlPointAtPoint:(NSPoint)p tolerance:(CGFloat)tol type:(NSInteger)controlPointType
{
	...
	if (inselem > 0) {
		...
		for (i = 0; i < m; ++i) {
			...
			if (pe == NSBezierPathElementMoveTo)
				firstPoint = ap[1];
			...
			if (i == inselem) {
				...
				if (pe == NSBezierPathElementCurveTo) {
					...
					if (pe == NSBezierPathElementClosePath)
						ap[1] = firstPoint;
...
// to count as a hit.
	...
	if (NSPointInRect(p, bb)) {
		...
		if (elem > 0) {
			...
			if (pretype == NSBezierPathElementCurveTo)
				lp[0] = lp[2];

			if (etype == NSBezierPathElementCurveTo) {
				// curve
			...
			} else if (etype != NSBezierPathElementMoveTo) {
				if (etype == NSBezierPathElementClosePath) {
...
- (NSRect)boundingBoxForElement:(NSInteger)elementIndex
{
	...
	if (lm == NSBezierPathElementMoveTo)
		return NSZeroRect;
	...
	if (lm == NSBezierPathElementCurveTo) {
		...
		if (pm == NSBezierPathElementCurveTo)
			ap[3] = pp[2];
		else
			ap[3] = pp[0];
	...
	} else if (lm == NSBezierPathElementClosePath) {
...
- (NSSet*)boundingBoxesForPartcode:(NSInteger)pc
{
	...
	if (e < [self elementCount]) {
		...
		if (j == 0 && e > 0 && lm == NSBezierPathElementCurveTo) {
