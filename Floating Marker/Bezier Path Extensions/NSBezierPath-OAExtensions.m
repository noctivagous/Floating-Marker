...
@implementation NSBezierPath (OAExtensions)
    ...
    // NCTVGS
    //
- (NSPoint) midpointForSegment:(int) segmentNum
    {
        OABezierPathPosition midpointPPosition;
        midpointPPosition.segment = segmentNum;
        midpointPPosition.parameter = 0.5;
        
        NSPoint midpointForSegment;
        midpointForSegment = [self getPointForPosition:midpointPPosition];
        
        return midpointForSegment;
    }
    


void _OAParameterizeLine(NSPoint *coefficients, NSPoint startPoint, NSPoint endPoint) {
...
void _OAParameterizeCurve(NSPoint *coefficients, NSPoint startPoint, NSPoint endPoint, NSPoint controlPoint1, NSPoint controlPoint2) {
    coefficients[0] = startPoint;
        ...
        for (elementIndex = 0; elementIndex < elementCount; elementIndex++) {
            ...
            switch (element) {
                ...
                case NSBezierPathElementLineTo: {
                    ...
                    NSPoint lineToPoint;
                    if (element == NSBezierPathElementClosePath)
...
- (BOOL)intersectsRect2:(NSRect)rect
{
    NSInteger count = [self elementCount];
    NSInteger i;
    NSPoint points[3];
    ...
    NSBezierPathElement firstElement = [self elementAtIndex:0 associatedPoints:points];
    if (firstElement != NSBezierPathElementMoveTo) {
        return NO;  // must start with a moveTo
    }
    ...
    for(i=1;i<count;i++) {
        ...
        switch(element) {
            case NSBezierPathElementMoveTo:
                ...
                break;
            case NSBezierPathElementClosePath:
                _OAParameterizeLine(line, currentPoint,startPoint);
                if (_straightLineIntersectsRect(line, rect)) {
                    return YES;
                }
                currentPoint = startPoint;
                ...
                break;
            case NSBezierPathElementLineTo:
                _OAParameterizeLine(line, currentPoint,points[0]);
                if (_straightLineIntersectsRect(line, rect)){
                    return YES;
                }
                currentPoint = points[0];
                break;
            case NSBezierPathElementCurveTo: {
                _OAParameterizeCurve(curve, currentPoint, points[2], points[0], points[1]);
                if (_curvedLineIntersectsRect(curve, rect, [self lineWidth]+1)) {
                    return YES;
                }
                currentPoint = points[2];
...
#define WEPSILON 1e-4
                    ...
                    if (selfIter.currentElt + 1 == otherIter.currentElt && intersectionsFound > 0) {
                        ...
                        if (i.leftParameterDistance < EPSILON && i.leftParameter >= (1 - WEPSILON) && i.rightParameter <= (WEPSILON)) {
                            intersectionsFound--;
                        }
                        ...
                        if (i.leftParameterDistance < EPSILON && i.leftParameter <= (WEPSILON) && i.rightParameter >= (1 - WEPSILON)) {
                ...
                for (intersectionIndex = 0; intersectionIndex < intersectionsFound; intersectionIndex++) {
                    ...
                    while (insertionPoint > 0 && intersections[insertionPoint - 1].left.parameter > segmentIntersections[intersectionIndex].leftParameter && intersections[insertionPoint - 1].left.segment >= selfIter.currentElt)
                    insertionPoint--;
        ...
        if (initializeSubpathWalkingState(&cursor, self, 0, YES)) {
            while (nextSubpathElement(&cursor)) {
                switch (cursor.what) {
                    case NSBezierPathElementClosePath:
                    case NSBezierPathElementLineTo:
                    if ((cursor.points[0].y <= point.y || cursor.points[1].y <= point.y) && (cursor.points[0].y > point.y || cursor.points[1].y > point.y) && (cursor.points[0].x <= point.x || cursor.points[1].x <= point.x)) {
                        ...
                        windingCount--;
                    } else if (cursor.points[0].y == point.y && cursor.points[1].y == point.y && (cursor.points[0].x <= point.x || cursor.points[1].x <= point.x) && (cursor.points[0].x >= point.x || cursor.points[1].x >= point.x)) {
                        hitCount++;
                    }
                    break;
                    case NSBezierPathElementCurveTo: {
...
- (struct pointInfo)_getPointInfoForPosition:(CGFloat)position
    {
        ...
        switch (element) {
            ...
            case NSBezierPathElementMoveTo: // PENDING: should probably skip this one
            case NSBezierPathElementLineTo: {
...
- (BOOL)isClockwise
    {
        ...
        for (elementIndex = 0; elementIndex < elementCount; elementIndex++) {
            ...
            if (elt == NSBezierPathElementMoveTo || elt == NSBezierPathElementLineTo)
        ...
        OBASSERT(hit);
        if (hit) {
            enum OAIntersectionAspect aspect
            = (edge.right.parameterDistance < 0) ? edge.right.secondAspect : edge.right.firstAspect;
            switch (aspect) {
                case intersectionEntryRight:
                return YES;
                break;
    ...
    for(i=1;i<count;i++) {
        ...
        switch(element) {
            case NSBezierPathElementMoveTo:
                ...
                break;
            case NSBezierPathElementClosePath:
                if ([self _straightStrokeHit:currentPoint :startPoint :point :position padding:padding]){
                    return i;
                }
                currentPoint = startPoint;
                needANewStartPoint = YES;
                break;
            case NSBezierPathElementLineTo:
                if ([self _straightStrokeHit:currentPoint :points[0] :point :position padding:padding]){
                    return i;
                }
                currentPoint = points[0];
                break;
            case NSBezierPathElementCurveTo:
                if ([self _curvedStrokeHit:point startPoint:currentPoint endPoint:points[2] controlPoint1:points[0] controlPoint2:points[1] position:position padding:padding]) {
                    return i;
                }
                currentPoint = points[2];
                break;
...
@implementation NSBezierPath (PrivateOAExtensions)
    ...
    static BOOL drangeIntersectsDrange(double r1start, double r1length, double r2start, double r2length)
    {
        ...
        return ((r2start <= (r1start + r1length) || (r2start + r2length <= (r1start + r1length))) && (r2start >= r1start || (r2start + r2length >= r1start)));
    }
...
(x) = high;      \
    ...
    static BOOL parameterizedMonotonicCurveBoundsIntersect(const NSPoint* curve1, const NSPoint* curve2)
    {
        ...
        return (drangeIntersectsDrange(curve1[0].x, delta1X, curve2[0].x, delta2X) && drangeIntersectsDrange(curve1[0].y, delta1Y, curve2[0].y, delta2Y));
    }
...
static inline void splitParameterizedCurve(const NSPoint *c, NSPoint *o, double start, double len)
{
    ...
    NSInteger intersectionsBetweenCurveAndLine(const NSPoint* c, const NSPoint* a, struct intersectionInfo* results)
    {
        ...
        xcubic[0] = c[0].x - a[0].x;
        ycubic[0] = c[0].y - a[0].y;
        for (i = 1; i < 4; i++) {
            xcubic[i] = c[i].x;
            ycubic[i] = c[i].y;
        }
        ...
        for (i = 0; i < count; i++) {
            ...
            switch (multiplicity[i]) {
                case 1: {
                    ...
                    enum OAIntersectionAspect aspect
                    = (dy < 0 ? intersectionEntryRight : (dy > 0 ? intersectionEntryLeft : intersectionEntryAt)); // Note the aspect is from the curve's point of view
                ...
                case 3: {
                    ...
                    enum OAIntersectionAspect aspect
                    = (ycubic[3] < 0 ? intersectionEntryRight : (ycubic[3] > 0 ? intersectionEntryLeft : intersectionEntryAt));
                    results[resultCount].leftEntryAspect = aspect;
                    results[resultCount].leftExitAspect = aspect;
                    break;
                }
...
#define dlog(expr) /* */
    ...
    NSInteger intersectionsBetweenCurveAndSelf(const NSPoint* c, struct intersectionInfo* results)
    {
        ...
        if (c[2].x * c[2].x < 3 * c[1].x * c[3].x || c[2].y * c[2].y < 3 * c[1].y * c[3].y)
        return 0;
        ...
        enum OAIntersectionAspect aspect
        = (crossproduct > 0) ? intersectionEntryRight : (crossproduct < 0) ? intersectionEntryLeft : intersectionEntryAt;
...
#define MEPSILON 1e-8
        ...
        while (count1 > 0 && count2 > 0) {
            ...
            if (buf[0].leftEntryAspect == buf[count1].leftEntryAspect && buf[0].leftExitAspect == buf[count1].leftExitAspect && fabs(buf[0].leftParameter - buf[count1].leftParameter) < MEPSILON && fabs(buf[0].rightParameter - buf[count1].rightParameter) < MEPSILON && fabs(buf[0].leftParameterDistance - buf[count1].leftParameterDistance) < MEPSILON && fabs(buf[0].rightParameterDistance - buf[count1].rightParameterDistance) < MEPSILON) {
                                          ...
                                          double bloom)
    {
        ...
        NSInteger ix;
        for (ix = 0; ix < 4; ix++) {
            errVectorX[ix] = c1[ix].x - c2[ix].x;
            errVectorY[ix] = c1[ix].y - c2[ix].y;
        }
                                                                ...
                                                                struct intersectionInfo* previousResults)
    {
        ...
        while (previousResults < results) {
            ...
            if (previousResults->leftParameterDistance != 0 && previousResults->leftParameter + previousResults->leftParameterDistance >= c1Low)
            break;
            
            if (previousResults->rightParameterDistance != 0 && (previousResults->rightParameter >= c2Low || previousResults->rightParameter + previousResults->rightParameterDistance >= c2Low))
            break;
    ...
    static NSInteger coalesceExtendedIntersections(struct intersectionInfo* results, NSInteger found)
    {
        ...
        for (i = 0; i + 1 < found; i++) {
            for (j = i + 1; j < found; j++) {
                ...
                if (drangeIntersectsDrange(results[i].leftParameter, results[i].leftParameterDistance, results[j].leftParameter, results[j].leftParameterDistance) && drangeIntersectsDrange(results[i].rightParameter, results[i].rightParameterDistance, results[j].rightParameter, results[j].rightParameterDistance) && signbit(results[i].rightParameterDistance) == signbit(results[j].rightParameterDistance)) {
...
- (BOOL)_curvedIntersection:(CGFloat*)length time:(CGFloat*)tim curve:(NSPoint*)c line:(NSPoint*)a
    ...
    BOOL initializeSubpathWalkingState(subpathWalkingState* s, NSBezierPath* p, NSInteger startIndex, BOOL implicitClosepath)
    {
        ...
        if (s->what != NSBezierPathElementMoveTo) {
            OBASSERT_NOT_REACHED("Bezier path element should be NSBezierPathElementMoveTo but isn't");
            return NO;
        }
    ...
    BOOL nextSubpathElement(subpathWalkingState* s)
    {
        ...
        if (s->currentElt >= s->elementCount) {
            ...
            if (s->possibleImplicitClosepath) {
                s->what = NSBezierPathElementClosePath;
    ...
    {
        if (s->what == NSBezierPathElementClosePath)
        return NO;
        ...
        if (!(s->possibleImplicitClosepath)) {
            if (nextEltIndex >= s->elementCount) {
                ...
                if (nextOp == NSBezierPathElementMoveTo)
                return NO;
            }
        }
    ...
    void repositionSubpathWalkingState(subpathWalkingState* s, NSInteger toIndex)
    {
        ...
        OBASSERT(s->what == NSBezierPathElementLineTo || s->what == NSBezierPathElementCurveTo || s->what == NSBezierPathElementClosePath);
    }
...
- (BOOL)_curvedStrokeHit:(NSPoint)point startPoint:(NSPoint)startPoint endPoint:(NSPoint)endPoint controlPoint1:(NSPoint)controlPoint1 controlPoint2:(NSPoint)controlPoint2 position:(CGFloat*)position padding:(CGFloat)padding
    {
...
- (BOOL)_straightStrokeHit:(NSPoint)startPoint
                        :(NSPoint)endPoint
                        :(NSPoint)point
                        :(CGFloat*)position
...
- (NSInteger)_segmentHitByPoint:(NSPoint)point position:(CGFloat*)position padding:(CGFloat)padding
    {
        ...
        for (i = 1; i < count; ++i) {
            ...
            switch (element) {
                ...
                break;
                case NSBezierPathElementCurveTo: {
                    // GPC: reject point if not in segment bounds - without this a bug in the _curvedStrokeHit method will prevent
