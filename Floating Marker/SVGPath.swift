...
{
    static func fromString(rawStringForValue: String) -> NSBezierPath.LineCapStyle
    {
        if rawStringForValue == "butt"
        {return NSBezierPath.LineCapStyle.butt}
        else if rawStringForValue == "round"
        {return NSBezierPath.LineCapStyle.round}
        else if rawStringForValue == "square"
        {
            return NSBezierPath.LineCapStyle.square
        }
...
{
    static func fromString(rawStringForValue: String) -> NSBezierPath.LineJoinStyle
    {
        if rawStringForValue == "bevel"
        {return NSBezierPath.LineJoinStyle.bevel}
        else if rawStringForValue == "miter"
...
extension NSBezierPath {
    func applyCommands(from svgPath: SVGPath, offset: CGFloat) {
        for command in svgPath.commands {
            ...
            case .line: line(to: point)
            case .quadCurve: addQuadCurve(to: point, controlPoint: controlPoint1)
            case .cubeCurve: curve(to: point, controlPoint1: controlPoint1, controlPoint2: controlPoint2)
            case .close: close()
            }
        }
    }
