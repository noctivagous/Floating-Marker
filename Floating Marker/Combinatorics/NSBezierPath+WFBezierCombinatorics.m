...
CGPoint vectorLeavingNodeForNextNode( WFBezierVertexNode * node, WFBezierVertexNode * vertices, WFBezierIndexChain * path, BOOL isPathA )
{
	...
	switch ( (isPathA)?next->elementA:next->elementB ) {
		case NSBezierPathElementClosePath:
			if ( ((isPathA)?node->elementA:node->elementB) == NSBezierPathElementMoveTo ) {
...
CGPoint vectorLeavingNodeForPreviousNode( WFBezierVertexNode * node, WFBezierVertexNode * vertices, WFBezierIndexChain * path, BOOL isPathA )
{
	...
	switch ( (isPathA)?node->elementA:node->elementB ) {
        case NSBezierPathElementMoveTo:
			if ( ((isPathA)?prev->elementA:prev->elementB) == NSBezierPathElementClosePath ) {
...
WFBezierVertexNode * originalVertexOverlappingNode( WFBezierVertexNode * origin, WFBezierVertexNode * vertices, WFBezierIndexChain * path, BOOL isPathA )
{
	NSUInteger flagTarget = (isPathA)?(WFBezierFlag_PathA):(WFBezierFlag_PathB);
	...
	while ( WFGeometryDistance( origin->intersectionPoint, result->intersectionPoint ) < WFGeometryPointResolution ) {
		if ( (result->flags & (WFBezierFlag_PathA|WFBezierFlag_PathB)) == flagTarget ) {
			return result;
		}
...
- (NSUInteger)WFGatherOperatingPointsForPath:(NSBezierPath *)pathB outPoints:(WFBezierVertexNode **)outPoints
{
	...
	if ( [self elementAtIndex:0] != NSBezierPathElementMoveTo ) return 0;
	if ( [pathB elementAtIndex:0] != NSBezierPathElementMoveTo ) return 0;
	// Check that both paths have correct winding type
    if ( [self windingRule] != NSWindingRuleNonZero ) return 0;
    if ( [pathB windingRule] != NSWindingRuleNonZero ) return 0;
	...
	for ( NSUInteger i = 1; i<[self elementCount]; i++) {
		...
		if ( elementA == NSBezierPathElementMoveTo ) {
		...
		for ( NSUInteger j = 1; j<[pathB elementCount]; j++) {
			...
			if ( elementB == NSBezierPathElementMoveTo ) {
				controlPtsB[0] = controlPtsB[1];
				continue;
			}
			if ( (elementA==NSBezierPathElementLineTo||elementA==NSBezierPathElementClosePath) && (elementB==NSBezierPathElementLineTo||elementB==NSBezierPathElementClosePath) ) {
				//
			...
			} else if ( elementA == NSBezierPathElementCurveTo && elementB == NSBezierPathElementCurveTo ) {
				//
				...
				CGFloat aHits[3];
				CGFloat bHits[3];
				if ( elementA == NSBezierPathElementLineTo || elementA == NSBezierPathElementClosePath ) {
			...
			if ( elementB == NSBezierPathElementLineTo ) {
				controlPtsB[0] = controlPtsB[1];
			} else {
		...
		if ( elementA == NSBezierPathElementLineTo ) {
			controlPtsA[0] = controlPtsA[1];
		} else {
		   ...
		   parallelIntersectRule:(void(^)(WFBezierVertexNode* node, WFBezierVertexNode * vertices, NSUInteger vertexCount, WFBezierIndexChain * pathA, WFBezierIndexChain * pathB, BOOL * canUsePathA, BOOL * canUsePathB))validTraveralFromParallelIntersection
{
	...
	for ( NSUInteger i = 0; i < vertexCount; i++ ) {
		...
		CGPoint testVec1A = CGPointZero;
		CGPoint testVec2A = CGPointZero;
		
		WFBezierVertexNode * nextNodeB = NULL;
		WFBezierVertexNode * previousNodeB = NULL;
		WFBezierVertexNode * originalB = NULL;
		CGPoint testVec1B = CGPointZero;
		CGPoint testVec2B = CGPointZero;
		...
		if ( originalA ) {
			if ( crossesBoundary || adjacentBoundaries || (coincidentBoundaries && !pathBOnEndpt) ) {
				...
				while ( WFGeometryDistance(node->intersectionPoint, nextNodeA->intersectionPoint) <= WFGeometryPointResolution ) {
                    if ( !nodeToSave && nextNodeA->elementA == NSBezierPathElementCurveTo && WFGeometryDistance( nextNodeA->controlPointsA[3], node->controlPointsA[0]) < WFGeometryPointResolution ) {
		...
		if ( originalB ) {
			if ( crossesBoundary || adjacentBoundaries || (coincidentBoundaries && !pathAOnEndpt) ) {
				...
				while ( WFGeometryDistance(node->intersectionPoint, nextNodeB->intersectionPoint) <= WFGeometryPointResolution ) {
                    if ( !nodeToSave && nextNodeB->elementB == NSBezierPathElementCurveTo && WFGeometryDistance( nextNodeB->controlPointsB[3], node->controlPointsB[0]) < WFGeometryPointResolution ) {
	...
	// Invalidate all the nodes we marked in the previous step
	for ( NSUInteger i = 0; i < vertexCount; i++ ) {
		WFBezierVertexNode * node = &vertices[i];
	...
	// Eliminate single point intersections between line/curve and curve/curve
	/*
	for ( NSUInteger i = 0; i < vertexCount; i++ ) {
		WFBezierVertexNode * node = &vertices[i];
		if ( !WFNodeIsOnBothPaths(node) ) continue;
		...
		BOOL pathAIntersect = node->elementA == NSBezierPathElementCurveTo && fabs(node->pathA_t-round(node->pathA_t)) > WFGeometryParametricResolution;
		BOOL pathBIntersect = node->elementB == NSBezierPathElementCurveTo && fabs(node->pathB_t-round(node->pathB_t)) > WFGeometryParametricResolution;
		
		if ( pathAIntersect ) {
			WFBezierVertexNode * next = nodeOnIndexPathAfterNode( node, vertices, indexedPathA, YES );
			WFBezierVertexNode * prev = nodeOnIndexPathBeforeNode( node, vertices, indexedPathA, YES );
			if ( (WFNodeIsOnBothPaths(next) && (floor(next->pathA_t) == floor(node->pathA_t))) || !WFNodeIsOnBothPaths(next) ) {
				if ( (WFNodeIsOnBothPaths(prev) && (floor(prev->pathA_t) == floor(node->pathA_t))) || !WFNodeIsOnBothPaths(prev) ) {
					CGPoint v1 = vectorLeavingNodeForNextNode(node, vertices, indexedPathA, YES);
					CGPoint v2 = vectorLeavingNodeForNextNode(node, vertices, indexedPathB, NO);
					WFGeometryNormalizeVector(&v1);
					WFGeometryNormalizeVector(&v2);
					if ( abs(v1.x*v2.x + v1.y*v2.y) > 1.0-WFGeometryAngularResolution ) {
						node->flags |= WFBezierFlag_PointInvalid;
					}
				}
			}
		}
		if ( pathBIntersect && !(node->flags & WFBezierFlag_PointInvalid) ) {
			WFBezierVertexNode * next = nodeOnIndexPathAfterNode( node, vertices, indexedPathB, NO );
			WFBezierVertexNode * prev = nodeOnIndexPathBeforeNode( node, vertices, indexedPathB, NO );
			if ( (WFNodeIsOnBothPaths(next) && (floor(next->pathB_t) == floor(node->pathB_t))) || !WFNodeIsOnBothPaths(next) ) {
				if ( (WFNodeIsOnBothPaths(prev) && (floor(prev->pathB_t) == floor(node->pathB_t))) || !WFNodeIsOnBothPaths(prev) ) {
					CGPoint v1 = vectorLeavingNodeForNextNode(node, vertices, indexedPathA, YES);
					CGPoint v2 = vectorLeavingNodeForNextNode(node, vertices, indexedPathB, NO);
					WFGeometryNormalizeVector(&v1);
					WFGeometryNormalizeVector(&v2);
...
- (NSBezierPath *)WFUnionWithPath:(NSBezierPath *)path
{
	...
	pointFinder = ^(WFBezierVertexNode * vertices, NSUInteger vertexCount, WFBezierIndexChain * indexedPathA, WFBezierIndexChain * indexedPathB, BOOL * isPathA)
	{
		for ( NSUInteger i = 0; i < vertexCount; i++ ) {
			...
			if ( WFNodeIsOnBothPaths(node) ) {
				...
				if ( node->elementA == NSBezierPathElementCurveTo && WFNodeIsOnBothPaths(nextNodeA) && nextNodeA->elementA == NSBezierPathElementCurveTo  ) {
                ...
                if ( node->elementB == NSBezierPathElementCurveTo && WFNodeIsOnBothPaths(nextNodeB) && nextNodeB->elementB == NSBezierPathElementCurveTo  ) {
	...
	parallelIntersectRules = ^(WFBezierVertexNode* node, WFBezierVertexNode *vertices, NSUInteger vertexCount, WFBezierIndexChain *pathA, WFBezierIndexChain *pathB, BOOL *canUsePathA, BOOL *canUsePathB)
	{
		...
		if ( *canUsePathA && *canUsePathB ) {
            ...
            if ( nextNodeA->elementA == NSBezierPathElementCurveTo && nextNodeB->elementB != NSBezierPathElementCurveTo ) {
				*canUsePathB = NO;
            } else if ( nextNodeA->elementA != NSBezierPathElementCurveTo && nextNodeB->elementB == NSBezierPathElementCurveTo ) {
				*canUsePathA = NO;
			}
		}
	};
...
- (NSString *)WFTestCaseDump
{
	...
	for ( NSUInteger i = 0; i < [self elementCount]; i++) {
		...
		switch ( element ) {
            case NSBezierPathElementLineTo:
				[result appendString:@"XCTAssertTrue( element == NSBezierPathElementLineTo, @\"Incorrect element\" );\n"];
				break;
            case NSBezierPathElementCurveTo:
				[result appendString:@"XCTAssertTrue( element == NSBezierPathElementCurveTo, @\"Incorrect element\" );\n"];
				break;
            case NSBezierPathElementMoveTo:
				[result appendString:@"XCTAssertTrue( element == NSBezierPathElementMoveTo, @\"Incorrect element\" );\n"];
				break;
			case NSBezierPathElementClosePath:
				[result appendString:@"XCTAssertTrue( element == NSBezierPathElementClosePath, @\"Incorrect element\" );\n"];
				break;
		}
